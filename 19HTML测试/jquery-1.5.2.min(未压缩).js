    ...
    (function () {
        ...
        jQuery(function () {
            ...
            if (document.defaultView && document.defaultView.getComputedStyle) {
                ...
                //edit 
                var tempComputedStyle = document.defaultView.getComputedStyle(div, null);
                if (tempComputedStyle != null) {
                    jQuery.support.reliableMarginRight = (parseInt(tempComputedStyle.marginRight, 10) || 0) === 0;
                }
            }
				...
				l = checkSet.length;
                    ...
                    if (isPartStr && !rNonWord.test(part)) {
                        ...
                        for (; i < l; i++) {
                            ...
                            if (elem) {
                                var parent = elem.parentNode;
                                if (parent == null || parent.nodeName == undefined) {
                                    //edit
                                    checkSet[i] = false;
                                } else {
    ...
    jQuery.extend({
        ...
        ajax: function (url, options) {
            ...
            function done(status, statusText, responses, headers) {
                ...
                // Success/Error
                if (isSuccess) {
                    if (!IsAjaxAbort_Pmway(status, response)) {
...
//jquery.json-2.3.min.js 沈瑞华 2013-03-18
(function ($) {
...
(function ($) {
    jquery_function_select();
    jquery_function_tableresizer();
})(jQuery);

//#region select
function jquery_function_select() {
...
jQuery.fn.selectedsize = function () {
    if (jQuery(this).get(0).options != null)
...
jQuery.fn.getSelectedText2 = function (value) {
    var isExist = false;
    var count = this.selectedsize();
    for (var i = 0; i < count; i++) {
        if (jQuery(this).get(0).options[i].value == value) {
            return jQuery(this).get(0).options[i].text;
        }
    }
}
    ...
    if (this.selectedsize() == 0) {
        ...
        for (var i = 0; i < count; i++) {
            if (jQuery(this).get(0).options[i].selected == true) {
                value += jQuery(this).get(0).options[i].value + ",";
            }
        }
...
jQuery.fn.setSelectedValue = function (value) {
    ...
    if (value.indexOf(",") > -1) {
        ...
        for (var j = 0; j < avalue.length; j++) {
            for (var i = 0; i < count; i++) {
                if (jQuery(this).get(0).options[i].text == avalue[j]) {
...
jQuery.fn.setSelectedValue2 = function (value) {
    if (value.indexOf(",") > -1) {
        var avalue = value.split(",");
        var count = this.selectedsize();
        for (var j = 0; j < avalue.length; j++) {
            for (var i = 0; i < count; i++) {
                if (jQuery(this).get(0).options[i].value == avalue[j]) {
                    jQuery(this).get(0).options[i].selected = true;
                }
            }
        }
...
//#region tableresizer.js
function jquery_function_tableresizer() {
    $.fn.tableresizer = function (options) {
        return;

        $.fn.tableresizer.defaults = { col_border: "2px solid #666" };
        var opts = $.extend({}, $.fn.tableresizer.defaults, options);
        var args = arguments;
        var resize_columns = function (root) {
            var gridObj = GetGrid(root.children("table").attr("id"));
            var fieldId_InitWidth = $("#" + gridObj.gridId).find(".fieldId_0").width();
            gridObj.resize_Mark = false;
            gridObj.resize_header;
            gridObj.resize_tbl = root.children("table");
            gridObj.resize_tr = gridObj.resize_tbl.find("tr:first");
            gridObj.resize_newWidth;
            gridObj.resize_oldWidth;
            endresize = function (gObj) {
                if (gObj.resize_Mark) {
                    SaveColumnWidth(gridObj, gObj.resize_header, gObj.resize_oldWidth, gObj.resize_newWidth);
                    ToResetRoottbl();
                }
                gObj.resize_Mark = false;
                root.onselectstart = new Function("return true");
                root.children("table").css("cursor", "");
            };
            gridObj.resize_tr.mousemove(function (e) {
                gridObj = GetCurrGridRoot(e);
                if (gridObj.resize_Mark) {

                    var tempWidth = e.clientX - gridObj.resize_header.offset().left - 8;
                    var current_width = gridObj.resize_header.width();
                    var tempRootWidth = root.width();
                    var newRootWidth = GetContainerMaxWidth_PageCenter();
                    if (tempWidth > 1) {
                        if (tempWidth > current_width) {
                            gridObj.resize_header.css("width", tempWidth);
                        }
                        else {
                            if (tempWidth > 50) {
                                gridObj.resize_header.css("width", tempWidth - 4);
                                root.find("td:eq(" + gridObj.resize_header.index() + ")").css("width", tempWidth - 4);
                                var field_0 = 0;
                                field_0 = fieldId_InitWidth + tempWidth;
                                $("#" + gridObj.gridId).find(".fieldId_0").css("display", "").width(field_0);
                            }
                        }
                        if (tempWidth < 50) { tempWidth = 50; }
                        gridObj.resize_newWidth = tempWidth;
                        parent.$("#spanSize").html(parseInt(tempWidth)).show();
                    }
                    ...
                    if (e.target.nodeName == "TH") {
                        var tgt = $(e.target);
                        var dosize = (e.clientX - tgt.offset().left > tgt.width() - 4);
                        if (IsDisabledDrag(tgt)) {
                            dosize = false;
                        }
            ...
            gridObj.resize_tbl.mouseup(function (e) {
                endresize(gridObj);
                return false;
            });
            gridObj.resize_tbl.unbind("mouseleave").bind("mouseleave", function (e) {
                endresize(gridObj);
                return false;
            });
            gridObj.resize_tr.mousedown(function (e) {
                if (e.target.nodeName == "TH" && $(this).css("cursor") == "col-resize") {
                    gridObj.resize_header = $(e.target);
                    gridObj.resize_oldWidth = gridObj.resize_header.width();
                    gridObj.resize_Mark = true;
                    root.onselectstart = new Function("return false");
                }
                return false;
            });
            gridObj.resize_tr.mouseup(function (e) {
                endresize(gridObj);
                return false;
            });
        };
        return this.each(function () {
            if ($(this).parent().parent().html().indexOf("roottbl") == -1) {
                var maxWidth = 0;
                var item = $(this).parents(".desktopItem:first");
                if (item.attr("class") != undefined) {
                    maxWidth = $(this).attr("fixMaxWidth");
                }
                else {
                    maxWidth = GetContainerMaxWidth_PageCenter();
                }
                
                //E6UI调整，将roottbl设置为可调整的项，在展开折叠左侧面板时宽度会发生变化
                var root = $(this).wrap("<div class='roottbl' style='height:100%;' isNeedChangeWidth='true' />").parent();
                resize_columns(root);
            }
        });
    };
...
function GetCurrGridRoot(e) {
    return GetGrid($(e.target).parents(".listViewTable:first").attr("id"));
}

function SaveColumnWidth(gridObj, header, oldWidth, newWidth) {
    parent.$("#spanSize").hide();
    if (oldWidth == newWidth) {
        return;
    }
    var fieldId = header.attr("id").split('_')[1];
    var moduleId = 0;
    var $grid = $("#" + gridObj.gridId);

    var allthWidth = parseInt($grid.attr("AllThWidth")) + newWidth - oldWidth;
    $grid.attr("AllThWidth", allthWidth);
    ToShowAjaxStatus(false);
    if (newWidth != undefined) {
        if (parseInt(newWidth) < 50) { newWidth = 50; }
        var data = fieldId + "_" + parseInt(newWidth);
        SaveFieldsWidth(data, gridObj, true);
    }
}

function IsDisabledDrag(tdObj) {
    return tdObj.attr("isfixedwidth") == "true";
}

function ToResetRoottbl() {
    return;

    var gridId = null;
    var gridObj = null;
    $(".roottbl .listViewTable").each(function (i) {
        gridId = $(this).attr("id");
        gridObj = GetGrid(gridId);
        gridObj.JustWidth();
        if (IsIE(8)) {
            var maxWidth = GetContainerMaxWidth_PageCenter();
            if (gridObj != null && gridObj.isSelect) {
                maxWidth = 848;
            }
            $(".pageCenter_Content").width(maxWidth);
        }
    });
}
//#endregion
    ...
    $.fn.jFixedtable = function (options) {
        ...
        return this.each(function () {
            ...
            this.ToFixTb = function () {
                ...
                var tds = $table.find("tbody tr.tdout").eq(this.headerRows).find("td");
                ...
                this.sData.css({
                    ...
                    "overflow-x": options.overflow_x,
                    "overflow-y": options.overflow_y
                });
